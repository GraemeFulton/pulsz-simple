...
const GameCard: React.FC<GameCardProps> = ({ game, onClick }) => {
  ...
  return (
    <div 
      ...
      className="group cursor-pointer transition-all duration-300 hover:-translate-y-1 w-[135px] flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 dark:focus:ring-offset-gray-900 rounded-lg"
    >
      <div className="relative rounded-xl group-hover:shadow-2xl transition-shadow shadow-md overflow-hidden aspect-[2/3]">
        {!isLoaded && <div className="absolute inset-0 bg-gray-200 dark:bg-gray-700 animate-pulse"></div>}
        <img
          src={game.imageUrl}
          alt={game.title}
      ...
      <div className="pt-2">
        <div className="flex items-center space-x-1.5">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-200 font-medium"><span className="font-bold">{game.playingCount.toLocaleString()}</span> playing</p>
        </div>
      </div>
    </div>
  );
