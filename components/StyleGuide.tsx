...
const Section: React.FC<{ title: string; children: React.ReactNode; id: string; }> = ({ title, children, id }) => (
    <section id={id} className="mb-12 scroll-mt-24">
      <h2 className="text-3xl font-semibold border-b-2 border-primary/20 pb-2 mb-6 text-gray-800 dark:text-gray-200">{title}</h2>
...
const SubSection: React.FC<{ title: string; description?: string; children: React.ReactNode }> = ({ title, description, children }) => (
    <div>
        <h3 className="text-xl font-semibold mb-1 text-gray-700 dark:text-gray-300">{title}</h3>
        {description && <p className="text-gray-500 dark:text-gray-400 mb-4 max-w-2xl">{description}</p>}
        <div className="bg-gray-100/50 dark:bg-gray-800/50 p-6 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-wrap items-center gap-6">
            {children}
        </div>
    </div>
);

const SidebarNavItem: React.FC<{ item: NavItem; isActive: boolean; isCollapsed: boolean; }> = ({ item, isActive, isCollapsed }) => (
    <li
...
const InfoDetail: React.FC<{ label: string; value: string | React.ReactNode; info?: boolean; }> = ({ label, value, info = false }) => (
    <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg text-center">
        ...
        <p className="text-sm sm:text-base font-bold text-gray-800 dark:text-gray-200 mt-1">{value}</p>
    </div>
);
...
const StaticModalContent: React.FC<{ game: Game }> = ({ game }) => {
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg relative overflow-hidden">
              ...
              <div className="p-6 sm:p-8 space-y-6">
                  <div className="text-center">
                      ...
                      <p className="mt-2 text-gray-600 dark:text-gray-400">Play this awesome game and win great prizes!</p>
                  </div>
                  ...
                  <div className="space-y-4">
                      <div className="bg-gray-100 dark:bg-gray-900 rounded-full p-1 flex">
                          <button 
                              onClick={() => setCoinType('GC')}
                              className={`w-1/2 py-2 text-sm font-semibold rounded-full transition-colors ${coinType === 'GC' ? 'bg-white dark:bg-gray-700 text-gray-800 dark:text-white shadow' : 'text-gray-500'}`}
                          >
                              Gold Coins
                          </button>
                          <button 
                              onClick={() => setCoinType('SC')}
                              className={`w-1/2 py-2 text-sm font-semibold rounded-full transition-colors ${coinType === 'SC' ? 'bg-white dark:bg-gray-700 text-gray-800 dark:text-white shadow' : 'text-gray-500'}`}
                          >
                              Sweepstakes Coins
                          </button>
                      </div>
  ...
  return (
    <div className="bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-300 flex flex-col h-screen overflow-hidden">
      {isModalOpen && <GameDetailModal game={epicJokerGame} onClose={() => setIsModalOpen(false)} />}
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700 p-4 flex justify-between items-center shrink-0 z-50">
        <h1 className="text-2xl font-semibold text-gray-800 dark:text-gray-200">Brand Book & Style Guide</h1>
          ...
          <Section id="logo" title="Logo">
              <SubSection title="Logotype Variations" description="The primary brand logo. The appropriate version should be used to ensure high contrast against the background.">
                  <div className="flex flex-col sm:flex-row items-center gap-8">
                      ...
                      <div className="dark p-8 bg-gray-900 rounded-xl border border-gray-700 text-center">
                          <Logo className="h-10" />
                          <p className="text-sm mt-4 text-gray-400">Dark Mode (Dark BG)</p>
                      </div>
                  </div>
              </SubSection>
              <SubSection title="Icon Tile" description="The primary brand icon, used for favicons, app icons, and other compact branding scenarios.">
                  <div className="p-8 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 text-center">
                      <div className="relative h-20 w-20 rounded-2xl mx-auto">
                        ...
                        /> */}
                           <svg className="h-20 w-20 " viewBox="0 0 682 682" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<rect width="682" height="682" fill="white"/>
                      ...
                      <p className="text-sm mt-4 text-gray-600 dark:text-gray-400">Primary Icon</p>
                  </div>
              </SubSection>
          </Section>
          <Section id="colors" title="Colors">
              <SubSection title="Palette" description="The core color palette defines the brand's identity. Functional colors are used for specific actions like gameplay buttons.">
                  <div className="text-center">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">#fe139c</p>
                  </div>
                  <div className="text-center">
                      <div className="w-24 h-24 rounded-xl bg-accent mb-2 shadow-md"></div>
                      <p className="font-semibold">Accent</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400">#c71cb8</p>
                  </div>
                  <div className="text-center">
                      <div className="w-24 h-24 rounded-xl bg-gray-800 dark:bg-gray-200 mb-2 shadow-md border border-gray-200 dark:border-gray-700"></div>
                      <p className="font-semibold">Text</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400">Gray Tones</p>
                  </div>
                  <div className="text-center">
                      <div className="w-24 h-24 rounded-xl bg-white dark:bg-gray-900 mb-2 shadow-md border border-gray-200 dark:border-gray-700"></div>
                      <p className="font-semibold">Background</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400">White / Dark</p>
                  </div>
                  <div className="text-center">
                      <div className="w-24 h-24 rounded-xl bg-orange-400 mb-2 shadow-md"></div>
                      <p className="font-semibold">Orange</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400">bg-orange-400</p>
                  </div>
                  <div className="text-center">
                      <div className="w-24 h-24 rounded-xl bg-green-500 mb-2 shadow-md"></div>
                      <p className="font-semibold">Green</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400">bg-green-500</p>
                  </div>
              </SubSection>
              <SubSection title="Functional Gradients" description="Gradients are used for primary call-to-actions and promotional materials to draw attention. Specific functional gradients are designated for key user actions.">
                  <div className="text-center">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">For registration, social, etc.</p>
                  </div>
                  <div className="text-center">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">For social media banners.</p>
                  </div>
                  <div className="text-center">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">Gold Coin actions.</p>
                  </div>
                  <div className="text-center">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">Gameplay buttons.</p>
                  </div>
              </SubSection>
          </Section>
          ...
          <Section id="typography" title="Typography">
              <SubSection title="Font Roles" description="The typography system is built on a clear separation of roles to balance brand personality with user interface legibility.">
                  <div className="flex flex-col md:flex-row w-full gap-6">
                      <div className="w-full md:w-1/2">
                          ...
                          <p className="font-sans text-gray-600 dark:text-gray-400">Used for elements that need a strong brand personality and visual impact.</p>
                          <ul className="list-disc list-inside mt-2 font-sans text-gray-600 dark:text-gray-400">
                      ...
                      <div className="w-full md:w-1/2 border-t md:border-t-0 md:border-l border-gray-200 dark:border-gray-700 pt-6 md:pt-0 md:pl-6">
                          ...
                          <p className="font-sans text-gray-600 dark:text-gray-400">Used for all standard UI text to ensure maximum readability and clarity across the application.</p>
                          <ul className="list-disc list-inside mt-2 font-sans text-gray-600 dark:text-gray-400">
              ...
              <SubSection title="Scale & Hierarchy" description="A consistent typographic scale is used to maintain rhythm and hierarchy. Major article or blog titles use `font-bold` for maximum impact. For other high-impact display text, such as in the hero jumbotron and promotional banners, `font-semibold` with `tracking-tight` is used. This pairing offers strong visual impact while preserving legibility. Standard UI section headings also use `font-semibold` for a clean, cohesive look.">
                  <div className="space-y-4 w-full">
                      <div>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Poppins Bold - 36px, tracking-tight</p>
                          <h1 className="text-4xl font-bold tracking-tight">Blog Post Title Heading</h1>
                      </div>
                      <div>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Poppins Semibold - 30px, tracking-tight</p>
                          <h2 className="text-3xl font-semibold tracking-tight">Display Heading (Banners, etc.)</h2>
                      </div>
                      <div>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Poppins Semibold - 20px</p>
                          <h3 className="text-xl font-semibold">Section Heading (Game Grids, etc.)</h3>
                      </div>
                      <div>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Inter Regular - 16px</p>
                      ...
                      <div>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Inter Medium - 14px</p>
              ...
              <SubSection title="Text Spacing (Kerning)" description="Headings use tight letter spacing (`tracking-tight`) to create a more compact and impactful display appearance. Body text maintains its default spacing for optimal readability.">
                  <div className="space-y-4 w-full">
                      <div>
                          <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Heading with Tight Tracking (Default)</p>
                          <h2 className="text-2xl font-semibold">Compact Heading</h2>
                      </div>
                      <div>
                          <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Heading with Normal Tracking (For Comparison)</p>
                          <h2 className="text-2xl font-semibold tracking-normal">Normal Spacing Heading</h2>
                      </div>
                      <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                          <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Body Text with Normal Tracking</p>
                      ...
                      {appIcons.map(({ name, component: IconComponent }) => (
                          <div key={name} className="flex flex-col items-center text-center gap-2">
                              <div className="w-16 h-16 bg-gray-200 dark:bg-gray-700 rounded-xl flex items-center justify-center">
                                  <IconComponent className="w-8 h-8 text-gray-800 dark:text-gray-200" />
                              </div>
                              <p className="text-xs font-mono text-gray-500 dark:text-gray-400">{name}</p>
                          </div>
                      ))}
                  </div>
              </SubSection>
          ...
          <Section id="buttons" title="Buttons & Tags">
              <SubSection title="Button Typography" description="Key action buttons use the 'Poppins' display font with a semibold weight for a confident but not overpowering appearance. UI-focused buttons (like icon buttons) use 'Inter' for legibility.">
                  <div className="flex flex-col items-start gap-4">
                      <p className="font-sans text-sm text-gray-500 dark:text-gray-400 w-full mb-2">Primary CTA Button (font-display: Poppins Semibold)</p>
                      <p className="font-sans text-xs text-gray-500 dark:text-gray-400 w-full -mt-3 mb-2">For primary actions on a page - try to limit to 1.</p>
                  ...
                  <div className="flex flex-col items-start gap-4">
                      <p className="font-sans text-sm text-gray-500 dark:text-gray-400 w-full mb-2">Ghost Button</p>
                      <p className="font-sans text-xs text-gray-500 dark:text-gray-400 w-full -mt-3 mb-2">For secondary actions that need to be available but not distracting. Features a transparent background with a keyline border.</p>
                  ...
                  <div className="flex flex-col items-start gap-4">
                      <p className="font-sans text-sm text-gray-500 dark:text-gray-400 w-full mb-2">Tertiary Button</p>
                      <p className="font-sans text-xs text-gray-500 dark:text-gray-400 w-full -mt-3 mb-2">For tertiary actions. No border, subtle gray background, and a slightly more prominent background on hover.</p>
                  ...
                  <div className="flex flex-col items-start gap-4">
                      <p className="font-sans text-sm text-gray-500 dark:text-gray-400 w-full mb-2">Icon Button (font-sans: Inter)</p>
                      <button className="p-2 rounded-xl border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
              ...
              <SubSection title="Gradient Action Buttons" description="High-impact buttons used for primary gameplay actions. They feature a two-line text structure to create a clear and compelling call-to-action.">
                  <div className="flex flex-col items-start gap-4">
                      <p className="font-sans text-sm text-gray-500 dark:text-gray-400 w-full mb-2">Gold Coin Play Button</p>
                  ...
                  <div className="flex flex-col items-start gap-4">
                      <p className="font-sans text-sm text-gray-500 dark:text-gray-400 w-full mb-2">Sweepstakes Coin Play Button</p>
              ...
              <SubSection title="Button Groups" description="Button groups are used to visually connect a set of related actions, making the UI clearer and more compact. Use them for controls like pagination, carousels, or filter toggles.">
                  <div className="w-full space-y-6">
                      <div>
                          <h4 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">Pill Group (for paired actions)</h4>
                          <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">Uses `rounded-full` for opposing controls like next/previous. The border between buttons is removed.</p>
                          <div className="flex items-center">
                              <button aria-label="Scroll left" className="p-2 rounded-l-full border border-r-0 border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
                                  <ChevronLeftIcon className="h-4 w-4 text-gray-600 dark:text-gray-300" />
                              </button>
                              <button aria-label="Scroll right" className="p-2 rounded-r-full border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
                                  <ChevronRightIcon className="h-4 w-4 text-gray-600 dark:text-gray-300" />
                              </button>
                          </div>
                      </div>
                      <div className="pt-6 border-t border-gray-200 dark:border-gray-700">
                          <h4 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">Standard Group (for multiple choices)</h4>
                          <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">Uses a container with a pill-shaped background for the active item. Ideal for filter toggles or view switchers. This provides a modern, clean way to handle selection states, clearly indicating the active filter within a visually cohesive group.</p>
                          <div className="bg-gray-100 dark:bg-gray-800 rounded-full p-1 inline-flex items-center space-x-1">
                              <button className="px-4 py-1.5 text-sm font-semibold font-sans text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-full transition-colors">
                                  My Bets
                              </button>
                              <button className="px-4 py-1.5 text-sm font-semibold font-sans text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-full transition-colors">
                                  All Bets
                              </button>
                              <button className="px-4 py-1.5 text-sm font-semibold font-sans text-gray-900 dark:text-white bg-white dark:bg-gray-700 rounded-full shadow-sm transition-colors">
                                  High Rollers
                              </button>
                              <button className="flex items-center space-x-2 px-4 py-1.5 text-sm font-semibold font-sans text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-full transition-colors">
              ...
              <SubSection title="Tags" description="Tags use the 'Inter' system font for clarity and legibility on component cards.">
                  ...
                  <div className="bg-white leading-loose border border-gray-200 dark:bg-gray-900 dark:border-gray-700 text-xs font-medium px-2 h-6 flex flex-col justify-center align-center rounded-xl">
          ...
          <Section id="components" title="Components">
              ...
              <SubSection title="Inputs" description="Standard input field used for search functionality.">
                  <div className="relative w-full max-w-md">
                      ...
                      <input type="text" placeholder="Search games" className="bg-white w-full pl-10 pr-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary transition dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400"/>
                  </div>
              </SubSection>
              <SubSection title="Menus" description="Sidebar navigation provides access to main app sections.">
                  <div className="flex gap-4">
                      {/* Expanded */}
                      <div className="w-64 dark:bg-gray-800 bg-white p-4 rounded-xl border dark:border-gray-700 border-gray-200">
                      ...
                      {/* Collapsed */}
                      <div className="w-24 dark:bg-gray-800 bg-white p-4 rounded-xl border dark:border-gray-700 border-gray-200">
